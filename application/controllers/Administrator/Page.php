...
class Page extends CI_Controller {
    ...
    public function batch()  {
        ...
        $data['content'] = $this->load->view('Administrator/add_batch', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function insert_batch()  {
        $mail = $this->input->post('batchname');
        $query = $this->db->query("SELECT Batch_Serial from tbl_batch where Batch_Serial = '$mail'");
        ...
        if($query->num_rows() > 0){
           ...
           echo json_encode($exists);
            ...
            $succ = $this->db->query("INSERT INTO tbl_batch (Batch_Serial, branch_id) VALUES ('$mail','1')");
		   if($succ){
			    $msg = true;
			    echo json_encode($msg);
		   }
    ...
    public function batchedit($id)  {
        ...
        $data['selected'] = $this->db->query("SELECT * from tbl_batch where id = '$id'")->row();
        $data['content'] = $this->load->view('Administrator/edit/batch_edit', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function batchupdate(){
        $id = $this->input->post('id');
        $serial= $this->input->post('batchname');

        $succ = $this->db->query("UPDATE tbl_batch SET Batch_Serial='$serial' WHERE id = '$id'");
        if($succ){
		    $msg = true;
            echo json_encode($msg);
        } 
    ...
    public function batchdelete(){
        $id = $this->input->post('deleted');
        $this->db->query("DELETE FROM tbl_batch WHERE id='$id'");
    } 

    public function getBatchs(){
        $batchs = $this->db->query("select * from tbl_batch")->result();
