	...
	<div class="row">
		<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
			<div class="form-group" style="margin-top:10px;">
				<label class="col-sm-1 col-sm-offset-1 control-label no-padding-right"> Select IMEI </label>
				<div class="col-sm-2"> 
					<v-select v-bind:options="imei_list" v-model="selectedIMEI" label="ps_imei_number" v-on:input="onChangeIMEI"></v-select>
				</div>
            </div> 
	...
	<div class="row">
		<div class="col-md-6">
			...
			<table class="table">
                   ...
                   <tr v-for="(imei, sl) in imeiReport.purchaseReport">
                       <td>Supplier Name : </td>
                       <td>{{  imei.Supplier_Name }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.purchaseReport">
                       <td>Supplier Code : </td>
                       <td>{{  imei.Supplier_Code }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.purchaseReport">
                       <td>Supplier Mobile : </td>
                       <td>{{  imei.Supplier_Mobile }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.purchaseReport">
                       <td>Purchase Invoice  : </td>
                       <td>{{  imei.invoiceNo }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.purchaseReport">
                       <td>Purchase Date  : </td>
                       <td>{{  imei.purchaseDate }}</td>
                   </tr>
            </table>
        </div>
        ...
        <h6>Sale Detail </h6> 
			<table class="table">
                   ...
                   <tr v-for="(imei, sl) in imeiReport.saleReport">
                       <td>Customer Name : </td>
                       <td>{{  imei.Customer_Name }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.saleReport">
                       <td>Customer Code : </td>
                       <td>{{  imei.Customer_Code }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.saleReport">
                       <td>Customer Mobile : </td>
                       <td>{{  imei.Customer_Mobile }}</td>
                   </tr>
                   <tr v-for="(imei, sl) in imeiReport.saleReport">
                       <td>Sale Invoice  : </td>
                       <td>{{  imei.invoiceNo }}</td>
...
<script>
	...
	new Vue({
		...
		methods:{
            async getIMEIS(){
                  await axios.post(`/get_imei_list`).then((res)=>{
                         this.imei_list = res.data;
                  })
            },
            onChangeIMEI(){
	       ...
	       this.serialModalStatus = false;
			...
			async getStock(){
				await axios.post('/get_imei_report',{imei:this.selectedIMEI.ps_imei_number})
				.then((res)=>{
					this.imeiReport = res.data;
				});
			},
